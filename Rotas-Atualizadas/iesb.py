...
for u, v, k, data in G.edges(keys=True, data=True):
    if "highway" in data:
        if data["highway"] in ["motorway", "primary"]:
            data["speed_kph"] = 80
        elif data["highway"] in ["secondary", "residential"]:
            data["speed_kph"] = 30
        else:
            data["speed_kph"] = 40
...
dest_node = ox.distance.nearest_nodes(G, X=-47.8925, Y=-15.7835)
...
print(f"Tempo de execução: {time_dijkstra:.6f} s")
...
print(f"\n[A* - Menor tempo de viagem]")
print(f"Tempo de execução: {time_astar:.6f} s")
print(f"Tempo estimado de viagem: {time_astar_total/60:.1f} min")
...
fig, ax = ox.plot_graph_routes(
    ...
    route_colors=['r', 'g'],   # vermelho = distância, verde = tempo
    route_linewidth=3,
    node_size=0
)
